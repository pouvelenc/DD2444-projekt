...
* **`main.py`**: Configures simulation parameters (lattice size, shots, noise models), runs sweeps (shot counts and error rates), and collects data.
...
**Prerequisites:** Python 3.12 (Strict) and Git.
...
1.  **Configure Parameters**:
    ...
    PARAMS = {
        ...
        "shots": [4096, 8192],         # list of shot counts
        ...
        "p_err_values": [0.001, 0.01]  # error rates for custom noise model
    }
    ```
...
3.  **Outputs**:
    ...
    * `counts_*.json`: Raw simulation data.
    * `*.gif`: Animated visualizations of the fluid flow.
    * `rmse_*.png`: Convergence plots comparing Noisy vs. ZNE-mitigated results.
